  ...
  return (
    <div className="relative w-full mx-auto mb-10 drop-shadow-2xl animate-fade-in-up">
      ...
      <div className="flex flex-col md:flex-row bg-slate-900 rounded-[2rem] overflow-hidden">
        ...
        <div className="flex-1 p-6 sm:p-8 relative text-white">
          ...
          <div className="flex justify-between items-start mb-8 relative z-10">
            <div>
              ...
              <span className="font-mono font-black text-2xl sm:text-3xl tracking-wider text-rose-500">{activeOrder.id}</span>
            </div>
            <span className="text-[10px] font-black bg-rose-500 text-white px-3 py-1.5 rounded-full flex items-center gap-1.5 uppercase shadow-lg shadow-rose-500/30">
              <span className="w-1.5 h-1.5 bg-white rounded-full animate-pulse"></span> {activeOrder.status}
            </span>
          </div>
          ...
          <div className="flex items-center gap-5 mb-8 relative z-10">
            ...
            <div>
              <h3 className="text-2xl font-black leading-tight mb-1">{activeOrder.item}</h3>
              <p className="text-slate-300 text-xs sm:text-sm font-medium flex items-center gap-1.5">
                <MapPin size={14} className="text-rose-500" /> {activeOrder.location || 'Lokasi belum diset'}
              </p>
            </div>
          </div>

          {/* Grid Jadwal (Ambil & Kembali) */}
          <div className="grid grid-cols-2 gap-4 bg-white/5 p-5 rounded-2xl border border-white/10 relative z-10">
            <div>
              ...
              <span className="font-bold text-white text-xs sm:text-sm">{activeOrder.startDate}</span>
            </div>
            <div>
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1">Drop-off (Kembali)</span>
              <span className="font-bold text-white text-xs sm:text-sm">{activeOrder.endDate}</span>
            </div>
          </div>
        </div>

        {/* ========================================= */}
        {/* PEMBATAS TIKET & LUBANG (UNTUK DESKTOP) */}
        {/* ========================================= */}
        <div className="hidden md:flex flex-col items-center justify-between bg-slate-900 relative w-8">
          {/* Lubang Atas (Sesuai warna background dashboard luar) */}
        ...
        {/* ========================================= */}
        {/* PEMBATAS TIKET & LUBANG (UNTUK MOBILE) */}
        {/* ========================================= */}
        ...
        {/* BAGIAN KANAN: POTONGAN TIKET (STUB) & QR CODE */}
        <div className="md:w-72 bg-slate-900 p-6 sm:p-8 flex flex-col items-center justify-center relative">
          ...
          <div className="bg-white p-4 rounded-3xl shadow-xl mb-6 relative overflow-hidden group">
            {/* Efek Garis Scanner Merah Animasi */}
          ...
          <div className="mt-8 pt-4 border-t border-white/10 w-full flex justify-center">
             <button onClick={() => navigate('/support')} className="text-[10px] font-black text-slate-400 hover:text-white flex items-center gap-1.5 transition-colors uppercase tracking-widest">
              <AlertTriangle size={14} className="text-rose-500" /> Bantuan Darurat
            </button>
          </div>
